...
const HeatmapChart: React.FC<HeatmapChartProps> = ({ data, metric, unit }) => {
  ...
  const cityCalendars = useMemo(() => {
    ...
    return cities.map((cityName) => {
      ...
      const cells: DayCell[] = [];
      const monthPositions: Map<
        string,
        { start: number; end: number; month: number; year: number }
      > = new Map();
      ...
      const months = Array.from(monthPositions.entries()).map(([, pos]) => ({
        label:
          pos.year !== firstYear ? `${MONTHS_HU[pos.month]}\n${pos.year}` : MONTHS_HU[pos.month],
            ...
            <div className="calendar-main">
              ...
              <div className="calendar-grid-area">
                <div
                  ...
                  style={{ gridTemplateColumns: `repeat(${weeks}, ${cellSize}px)` }}
                >
                  {cells.map((cell, i) => (
                    <div
                      key={i}
