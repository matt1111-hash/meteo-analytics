        ...
        try:
            ...
            if self.current_data:
                ...
                if dates:
                    ...
                    msg_box.setText(f"Részletes elemzés indítva:\n- Kezdő dátum: {start_date}\n- Befejező dátum: {end_date}\n- Összes nap: {total_days}\n- Anomália detektor: {detector_status}\n- Rekord számító: {calculator_status}")
                    msg_box.setStandardButtons(QMessageBox.Ok)
                    msg_box.exec()
            
        except Exception as e:
            logger.error(f"Részletes elemzés hiba: {e}")
    
    def _on_anomaly_settings_clicked(self) -> None:
        """⚙️ Anomália beállítások gomb eseménykezelő."""
        try:
            logger.info("Anomaly settings button clicked")
            
            if _anomaly_dialog_available and _anomaly_detector_available:
                if not self.settings_dialog:
                    self.settings_dialog = AnomalySettingsDialog(self)
                
                self.settings_dialog.settings_saved.connect(self._on_anomaly_settings_saved)
                self.settings_dialog.show()
            else:
                logger.warning("Anomaly settings dialog or detector not available")
                QMessageBox.warning(self, "Hiba", "Anomália beállítások nem érhetők el.")
    
    def _on_anomaly_settings_saved(self, settings: Dict[str, Any]) -> None:
        """Anomália beállítások mentésének kezelése."""
        try:
            logger.info("Anomaly settings saved, updating detector")
            if self.anomaly_detector:
                self.anomaly_detector.update_settings(settings)
                self._update_anomaly_display_from_detector(self.current_data.get('daily', {}))
        except Exception as e:
            logger.error(f"Anomaly settings save error: {e}")
