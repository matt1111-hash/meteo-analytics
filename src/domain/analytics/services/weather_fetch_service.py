...
class WeatherFetchService:
    ...
    def fetch_weather_data_dual_api_batch(
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            batch_results = self.process_dual_api_batch(batch, date)
    ...
    def process_dual_api_batch(self, batch: List[Dict[str, Any]], date: str) -> List[CityWeatherData]:
        """Process a batch in parallel and collect results."""
        batch_results: List[CityWeatherData] = []
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(self.fetch_single_city_weather_dual_api, city, date): city for city in batch}
    ...
    def fetch_single_city_weather_dual_api(self, city: Dict[str, Any], date: str) -> CityWeatherData:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if weather_data and len(weather_data) > 0:
                    ...
                    logger.debug(
                        "WIND DEBUG %s: windspeed=%s, windgusts=%s", city["city"], windspeed, windgusts
                    )
