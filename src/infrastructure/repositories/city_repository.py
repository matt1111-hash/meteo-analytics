...
class CityRepository(CityRepositoryProtocol):
    ...
    def __init__(
        ...
        self.hungarian_db_path = (
            hungarian_db_path
            or project_root / "data" / "hungarian_settlements.db"
        )
    ...
    def _apply_fallbacks(self) -> None:
        ...
        if not self.hungarian_db_path.exists():
            ...
            env_dir = os.environ.get("WEATHER_ANALYZER_DATA_DIR")
            env_path = (
                Path(env_dir) / "hungarian_settlements.db" if env_dir else None
            )
            if fallback.exists():
    ...
    def validate_paths(self) -> None:
        ...
        if not self.hungarian_db_path.exists():
            issues.append(
                "Hungarian settlements database missing: "
                f"{self.hungarian_db_path}"
            )
        if issues and not (
            self.db_path.exists() or self.hungarian_db_path.exists()
        ):
            detail = "\n".join(issues)
    ...
    def get_cities_by_names(self, city_names: List[str]) -> List[Dict[str, object]]:
        ...
        # Try global cities database first
        # Use subquery to get only the highest-population match per city name
        ...
        # Fallback to Hungarian database if available
        if self.hungarian_db_path.exists():
            ...
            return self._execute(self.hungarian_db_path, hun_query, params)
    ...
    def get_cities_for_region(
        ...
        if mapped_region == "Global":
            return self._query_global(limit)
        if mapped_region == "Hungary":
            if original_region in hungarian_mapping:
                return self._query_hungarian_region(
                    original_region, hungarian_mapping, limit
                )
            return self._query_hungary_all(limit)
        return self._query_countries(country_codes, limit)

    def _query_global(self, limit: int) -> List[Dict[str, object]]:
        base_select = (
            "SELECT city, country, country_code, lat, lon, population, "
            "meteostat_station_id, data_quality_score FROM cities "
    ...
    def _query_hungary_all(self, limit: int) -> List[Dict[str, object]]:
        base_select = (
            'SELECT city, country, country_code, lat, lon, population, '
            'meteostat_station_id, data_quality_score FROM cities '
    ...
    def _query_hungarian_region(
        ...
        return self._execute(self.hungarian_db_path, base_select, params)
    ...
    def autocomplete_city_name(self, query: str, limit: int = 20) -> List[Dict[str, object]]:
        ...
        Args:
            query: Partial city name to search for (case-insensitive)
            limit: Maximum number of results to return

        Returns:
            List of city dictionaries matching the query
        """
        ...
        if self.hungarian_db_path.exists():
            hun_results = self._execute_hungarian(self.hungarian_db_path, hun_query, params)
