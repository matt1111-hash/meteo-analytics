...
class SignalManager:
    ...
    def _connect_control_panel(self) -> None:
        ...
        if self.mw.control_panel:
            ...
            if hasattr(self.mw.control_panel, 'analysis_requested'):
                ...
                print("✅ SignalManager: ControlPanel.analysis_requested → AppController.handle_analysis_request CONNECTED")
            else:
            ...
            if hasattr(self.mw.control_panel, 'cancel_requested'):
                ...
                print("✅ SignalManager: ControlPanel.cancel_requested → AppController.stop_current_analysis CONNECTED")
            else:
    ...
    def _connect_app_controller(self) -> None:
        ...
        if hasattr(controller, 'analysis_started'):
            ...
            print("✅ SignalManager: AppController.analysis_started → MainWindow._on_analysis_started CONNECTED")
        ...
        if hasattr(controller, 'analysis_failed'):
            ...
            print("✅ SignalManager: AppController.analysis_failed → MainWindow._on_analysis_failed CONNECTED")

        # Elemzés megszakítva
        if hasattr(controller, 'analysis_cancelled'):
            controller.analysis_cancelled.connect(self.mw._on_analysis_cancelled)
            print("✅ SignalManager: AppController.analysis_cancelled → MainWindow._on_analysis_cancelled CONNECTED")
    ...
    def _connect_analytics_view(self) -> None:
        ...
        if self.mw.analytics_panel:
            ...
            if hasattr(self.mw.analytics_panel, 'multi_city_query_requested'):
                ...
                self.mw.analytics_panel.multi_city_query_requested.connect(debug_analytics_multi_city_query_requested)
                ...
                print("❌ SignalManager: AnalyticsView.multi_city_query_requested signal NOT FOUND!")
    ...
    def _connect_provider_status(self) -> None:
        ...
        print("✅ SignalManager: Controller.provider_selected → MainWindow._on_provider_selected CONNECTED")

        # Usage statistics frissítése
        controller.provider_usage_updated.connect(self.mw._on_provider_usage_updated)
        print("✅ SignalManager: Controller.provider_usage_updated → MainWindow._on_provider_usage_updated CONNECTED")

        # Warning events
        controller.provider_warning.connect(self.mw._on_provider_warning)
        print("✅ SignalManager: Controller.provider_warning → MainWindow._on_provider_warning CONNECTED")

        # Fallback notifications
        controller.provider_fallback.connect(self.mw._on_provider_fallback)
        print("✅ SignalManager: Controller.provider_fallback → MainWindow._on_provider_fallback CONNECTED")
    ...
    def _connect_results_panel(self) -> None:
        ...
        if self.mw.results_panel:
            ...
            if hasattr(self.mw.results_panel, 'export_requested'):
                ...
                print("✅ SignalManager: ResultsPanel.export_requested → MainWindow._handle_export_request CONNECTED")
            else:
            ...
            if hasattr(self.mw.results_panel, 'extreme_weather_requested'):
                self.mw.results_panel.extreme_weather_requested.connect(self.mw._show_extreme_weather)
                print("✅ SignalManager: ResultsPanel.extreme_weather_requested → MainWindow._show_extreme_weather CONNECTED")
            else:
                print("⚠️ SignalManager: ResultsPanel.extreme_weather_requested signal NOT FOUND - SKIPPING")
        else:
    ...
    def _connect_theme_manager(self) -> None:
        ...
        if hasattr(self.mw, 'theme_changed'):
            ...
            print("✅ SignalManager: MainWindow.theme_changed → MainWindow._propagate_theme_change CONNECTED")
