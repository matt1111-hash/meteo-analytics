...
class MainWindow(QMainWindow):
    ...
    def _connect_mvc_signals(self) -> None:
        ...
        if hasattr(self.control_panel, "provider_usage_updated"):
            self.control_panel.provider_usage_updated.connect(
                self._on_provider_usage_updated
            )
        ...
        if hasattr(self.results_panel, "extreme_weather_requested"):
            self.results_panel.extreme_weather_requested.connect(
                self._show_extreme_weather
            )

        # AnalyticsView signalok
        if hasattr(self.analytics_panel, "query_requested"):
            self.analytics_panel.query_requested.connect(
    ...
    def _handle_multi_city_weather_request(
        ...
        handle_multi_city_weather_request(
            self, analysis_type, region_id, start_date, end_date, params
        )
    ...
    def _on_multi_city_result_ready_for_views(
        self, result: "AnalyticsResult", query_type: str = "hottest_today"
    ) -> None:
