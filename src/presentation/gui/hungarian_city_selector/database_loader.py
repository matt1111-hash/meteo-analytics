...
class HungarianCityDatabaseLoader:
    ...
    def load_cities(
        self, error_signal: Signal, stats_update_callback: Callable[[str], None]
    ) -> List[HungarianCity]:
        """
    ...
    def calculate_city_stats(cities: List[HungarianCity]) -> str:
        ...
        stats_text += (
            f"  • Összlakosság: {population_sum:,} fő ({population_count} városban)\n"
        )
        ...
        for region, count in sorted(
            ...
            display_name = HungarianRegions.REGION_DISPLAY_NAMES.get(
                region, f"❓ {region}"
            )
