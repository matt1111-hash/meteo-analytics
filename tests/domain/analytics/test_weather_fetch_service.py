...
def test_fetch_weather_data_dual_api_batch_returns_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cities = [
        {"city": "Budapest", "country": "Hungary", "country_code": "HU", "lat": 47.5, "lon": 19.0}
    ]
...
def test_fetch_single_city_weather_dual_api_returns_empty_after_retries(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("time.sleep", lambda *_args, **_kwargs: None)
    client = _DummyWeatherClient(raise_error=True)
    service = _service(client)
    city = {"city": "Failtown", "country": "X", "country_code": "XX", "lat": 0.0, "lon": 0.0}
